 ...
 */
export function writeProgressState(toolName, description, cwd) {
...
export function readBridgeMessages(accountId, toUserId, sinceMs) {
  ...
  try {
    ...
    // The bridge stores messages as user/assistant pairs
    // We want the most recent user messages
    const results = [];
    for (const msg of messages) {
      if (msg.role === 'user' && msg.content) {
        // We don't have timestamps per-message in the conversation log
        // so just return the most recent user messages
